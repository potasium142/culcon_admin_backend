...
def get_last_7_days_revenue(db: Session) -> list[dict[str, float]]:
    ...
    for i in range(7):
        ...
        daily_revenue = db.query(func.sum(PaymentTransaction.amount))\
...
def get_last_6_months_revenue(db: Session) -> list[dict[str, float]]:
    ...
    for i in range(6):
        ...
        monthly_revenue = db.query(func.sum(PaymentTransaction.amount))\
                            .filter(func.extract('year', PaymentTransaction.create_time) == target_year)\
                            .filter(func.extract('month', PaymentTransaction.create_time) == target_month)\
                            .scalar() or 0.0
        revenues.append({"month": f"{target_year}-{target_month:02d}", "revenue": monthly_revenue})
    return revenues