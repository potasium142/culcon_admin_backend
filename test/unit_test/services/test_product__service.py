...
@pytest.mark.asyncio
@patch(
    "services.product_.cloudinary.upload", return_value="http://fakeurl.com/image.jpg"
)
...
async def test_product_creation_fail_invalid_name_too_long(mock_upload, db_session):
    ...
    with pytest.raises(
        HandledError,
        match="Product id is longer than 255, please trim down product name",
    ):
        await product_creation(
            prod_info=product_info,
            additional_images=additional_images,
...
async def test_mealkit_creation_success(mock_upload, db_session):
    ...
    result_ingredients = (
        (
            await db_session.execute(
                select(prod.MealkitIngredients).where(
                    prod.MealkitIngredients.mealkit_id == result["id"]
                )
            )
        )
        .scalars()
    ...
    rice_product = await db_session.execute(
        select(prod.Product).where(prod.Product.product_name == "Rice")
    )
    tofu_product = await db_session.execute(
        select(prod.Product).where(prod.Product.product_name == "Tofu")
    )
...
async def test_mealkit_creation_fail_no_ingredent_product_in_database(
    mock_upload, db_session
):
    # Start a transaction block to ensure rollback happens after the test
